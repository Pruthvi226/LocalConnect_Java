...
const LoginCustomer = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!username || !password) {
      setError('Credentials required for transmission.');
      return;
    }
    ...
    try {
      ...
      // Success redirection happens via AuthContext or manually here
      navigate('/search'); 
    } catch (err) {
      ...
      >
        <div className="bg-white rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(79,70,229,0.15)] border border-white p-8 lg:p-12 relative overflow-hidden">
          ...
          <div className="text-center mb-10">
            ...
            <h1 className="text-4xl font-black text-slate-900 mb-3 tracking-tight">Customer <span className="text-primary-600">Access.</span></h1>
            <p className="text-slate-500 font-medium">Powering your local service ecosystem</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Identity / Username</label>
          ...
          <div className="mt-12 pt-8 border-t border-slate-50">
            <div className="flex flex-col gap-4">
               <p className="text-center text-slate-400 font-bold text-sm">
                 ...
                 <Link to="/register/customer" className="text-primary-600 hover:underline">Create ID</Link>
               </p>
               <div className="flex items-center gap-4">
                  <div className="h-px bg-slate-100 flex-1"></div>
                  <span className="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">Matrix Switch</span>
                  <div className="h-px bg-slate-100 flex-1"></div>
               </div>
               <Link 
                to="/login/provider" 
        ...
        <p className="text-center mt-8 text-slate-400 text-xs font-medium">
          Secured by ProxiSense Hyperlocal Encryption Standard
        </p>
      </motion.div>
    </div>
  );
